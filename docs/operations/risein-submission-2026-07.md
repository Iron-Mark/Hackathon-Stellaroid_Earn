...
- **Wallet-less guided demo** (stellaroid.tech/demo): the full
  register → verify → escrow → payout story on real seeded testnet data — a
  released 25 XLM escrow and a live funded one — with per-step stellar.expert
  audit links. Judges need no extension, and it works on a phone.
...
3. https://stellaroid.tech/opportunity — both escrow exhibits live
   (#0 released, #1 funded).
...
Everything runs on Stellar **testnet** as an early-access pilot: no mainnet
claims, no fabricated metrics, no fake customers. The demo exhibits are
labeled as team-seeded, and every number above links to a public record.
