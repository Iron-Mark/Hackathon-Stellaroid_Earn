...
`v4.stellaroid.tech` is the active August showcase domain. Map it to `august-monthly-builder` in Vercel and add the `v4` CNAME at the DNS host. Keep `v3.stellaroid.tech` mapped to `july-monthly-builder` as the frozen July snapshot.
