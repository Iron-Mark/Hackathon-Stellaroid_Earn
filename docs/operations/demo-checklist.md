...
Lighthouse (mobile, throttled, 2026-07-10 — after the stellar-sdk lazy-load): `/` = 72, `/app` = 60, `/demo` = 82; TBT 50–260 ms everywhere; CLS ≈ 0 on `/` and `/demo`. Known watch item: `/app` CLS 0.139 under throttling (font swap suspected; not reproducible on warm loads) — check Speed Insights field data before chasing it.
