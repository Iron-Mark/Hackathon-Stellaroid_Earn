...
| 5 | **Dependency audit** | PASS | `npm audit`: 0 critical, 0 high, 0 moderate. The 2026-07-25 sweep patched every high and moderate that had surfaced since the prior review: Next.js (8 advisories: SSRF, DoS, cache confusion) bumped in range to 15.5.21; `sharp` to 0.35.3 (libvips CVEs), forced by override so Next's bundled copy is covered too; `postcss` override raised to 8.5.23; `fast-uri` to 3.1.4 and `hono` to 4.12.32 (both transitive under the MCP SDK); and `@hono/node-server` overridden to 2.x, clearing its Windows serve-static path traversal even though this app reaches the MCP SDK through a Next route handler and never runs its static-file server. 23 low-severity alerts remain, all chaining to a single unpatched advisory (`elliptic <=6.6.1`, "risky cryptographic primitive") reached only through Stellar Wallets Kit's bundled Trezor / HOT / NEAR wallet SDKs, which this app never executes. `elliptic 6.6.1` is the latest published version, so no non-breaking upstream fix exists; npm's only remedy is a breaking Stellar Wallets Kit downgrade. Accepted and tracked; revisit when elliptic ships a patch or the kit slims its tree. |
